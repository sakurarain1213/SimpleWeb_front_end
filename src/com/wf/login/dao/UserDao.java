...
public class UserDao {
	 ...
	 */
	/*table name  用户表名*/
		String mytablename="testuser";

	public boolean insert(User user) {
		Connection con = DBUtil.getConnection();
		PreparedStatement pstmt =null;
		String sql = "INSERT INTO "+mytablename+"(username,nickname,password,gender,phone,email,address) VALUES(?,?,?,?,?,?,?)";
		boolean res = false;
		try {
	...
	public boolean delete(Integer userid) {
		Connection con = DBUtil.getConnection();
		PreparedStatement pstmt =null;
		String sql = "delete from "+mytablename+" where userid= ?";
		boolean res = false;
		try {
			pstmt = con.prepareStatement(sql);
	...
	public boolean update(User user) {
		Connection con = DBUtil.getConnection();
		PreparedStatement pstmt =null;
		String sql = "update "+mytablename+" set username=?,nickname=?,password=?,gender=?,phone=?,email=?,address=? where userid= ?";
		boolean res = false;
		try {
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1,user.getUsername());
	...
	public User selectByUsername(String username){
		...
		String sql = "select * from "+mytablename+" where username=?";
		PreparedStatement pstmt =null;
		ResultSet rs;
		try {
			pstmt = con.prepareStatement(sql);
	...
	public User selectByUserId(Integer userid) {
        Connection con =DBUtil.getConnection();
		List<User> list = new ArrayList<>();
		String sql = "select * from "+mytablename+" where userid= ?";
		PreparedStatement pstmt =null;
		ResultSet rs;
		try {
			pstmt = con.prepareStatement(sql);
